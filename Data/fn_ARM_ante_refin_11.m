...
function [itotal_avg] = fn_ARM_ante_refin_11(obj, X, t, T, H, loan, fixperiod, margin_rate, initial_rate, ini_cap,  anu_cap, life_cap_rate, FRM_pmt,  yearly , FRM_endvalue_H, Discountrate, n_intsimul, po_short_rate, short_trend, vasicek_coeff);
   H_varsicek = H-1;
...
%     ARM_pmt_total=sum(ARM_pmt.*12);    
   ...
   Indexed_rate_mo=[];  %위에서 yearly로 구한 ARM inedexed rate를 monthly로 구하기 위해 매해 같은 값을 12개월씩 추가하여 만듬
    ...
    for mo=1: H *12
     ...
     shock1(end+1,1) = numberofshock / (H*12) ;
...
%     AVG_size_shock1(end+1,1) = mean(ARM_pmt)/FRM_pmt-1;  
    ...
    AVG_size_shock1(end+1,1) = mean(pmtshock1)/mean(FRM_pmt) -1 ; % 잘못구한것 saving 을 고려 안한 shock size
    AVG_size_shock12(end+1,1) = mean(pmtshock1)/mean(FRM_pmt) ;  %이걸로 사용
    ...
    AVG_size_shock2(end+1,1) = mean(pmtshock2)/mean(FRM_pmt) -1 ;  %saving 을 고려한 shock size  틀린거
    AVG_size_shock22(end+1,1) = mean(pmtshock2)/mean(FRM_pmt);   %% 맞는거
    ...
    AVG_size_shock3(end+1,1) = mean(pmtshock3)/mean(FRM_pmt);       
   ...
   %% FRM cost with saving
    ...
    for mo = 1:H *12
        ...
        if mo < H*12
        Efrmcost1(end+1,1) = ( FRM_pmt(mo)-ARM_pmt_monthly(mo) ) /  FRM_pmt(mo) * 100;  %NPV구하기 위한 net cashflow 값을 미리 구함
        else
        Efrmcost1(end+1,1) = ( (FRM_pmt(mo) + FRM_endvalue_H) - (ARM_pmt_monthly(mo) + ARM_endvalue_H) ) / FRM_pmt(mo) * 100 ;
        end
        
    end    
    
    Efrmcost(end+1,1) = mean( Efrmcost1 )  ; %PV의 경우 discount차이라도 있는데 이건 그냥 평균이여서 sa, non sa의 차이가 없다. 
