function [po_ARM_total] =fn_ARM_POST_6(short_rate, T, H, loan, fixperiod, margin_rate, initial_rate, ini_cap,  anu_cap, life_cap_rate, FRM_pmt, FRM_rate, FRM_endvalue_H, Discountrate);
...
%     AVG_size_shock = mean(ARM_pmt)/FRM_pmt - 1;
    ...
    AVG_size_shock1 = mean(pmtshock1)/FRM_pmt -1 ; %saving 을 고려 안한 shock size
    AVG_size_shock12 = mean(pmtshock1)/FRM_pmt;
    ...
    AVG_size_shock2 = mean(pmtshock2)/FRM_pmt -1 ;  %saving 을 고려한 shock size 틀린거
    AVG_size_shock22 = mean(pmtshock2)/FRM_pmt ;   %맞은것
    ...
    AVG_size_shock3 = mean(pmtshock3)/FRM_pmt;      %틀린거  
    error(end+1,1)=AVG_size_shock3-AVG_size_shock22;
    ...
    Efrmcost = mean( cashflow_net./FRM_pmt *100 ) ; %PV의 경우 discount차이라도 있는데 이건 그냥 평균이여서 sa, non sa의 차이가 없다. 
            
    %% E(FRM cost) saving 8/20버젼  
    
%     Efrmcost_sa = mean( cashflow_net2./FRM_pmt *100 ) ;
    ...
    po_ARM_total = [npv_all, shock1, shock2, AVG_size_shock1, risk_index, risk_ARM, ARM_IRR, AVG_size_shock2, AVG_size_shock12, AVG_size_shock22, AVG_size_shock3, error, npv_all2, Eshock_nosa, Eshock_sa, Efrmcost];    %14부터 시작
